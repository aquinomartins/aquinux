...
function viewTrending(){
  window.location.assign('/trending');
}
...
function renderEmbeddedPageView({ title, subtitle, src }){
  const frameId = `embedded-frame-${Math.random().toString(36).slice(2, 9)}`;
  return `
    <section class="spa-embedded-page">
      <header class="spa-embedded-page__header">
        <p class="subtle subtle--caps">${esc(title)}</p>
        <h1 class="heading heading--xl">${esc(title)}</h1>
        <p class="subtle">${esc(subtitle)}</p>
      </header>
      <div class="spa-embedded-page__frame-wrap">
        <div class="spa-embedded-page__status" data-embedded-status="${frameId}" role="status" aria-live="polite">
          Carregando página...
        </div>
        <iframe
          id="${frameId}"
          class="spa-embedded-page__frame"
          title="${esc(title)}"
          src="${esc(src)}"
          loading="lazy"
          referrerpolicy="same-origin"
        ></iframe>
        <p class="spa-embedded-page__fallback subtle">
          Se a página não aparecer automaticamente, <a href="${esc(src)}" target="_blank" rel="noopener noreferrer">abra o Mercado LMSR em uma nova aba</a>.
        </p>
      </div>
    </section>
  `;
...
function setupEmbeddedPageStatus(){
  const frame = document.querySelector('.spa-embedded-page__frame');
  if (!frame) return;

  const statusEl = document.querySelector(`[data-embedded-status="${frame.id}"]`);
  if (!statusEl) return;
...
function viewMateriais(){
  const view = document.getElementById('view');
  if (!view) return;
  view.className = 'spa-embedded-view';
  view.innerHTML = renderEmbeddedPageView({
    title: 'Materiais',
    subtitle: 'Bibliografia e materiais de referência para o planejamento interdisciplinar.',
    src: 'materiais.html',
  });
}
